...
export const pwnRequestCheckoutRule: Rule = ({ workflow }) => {
  if (
  ...
  const severity: Severity = hasAiAction ? "critical" : "high";
  ...
  return checkoutSteps.map((step) =>
    result({
      ...
      column: step.column,
      message:
        "A pull_request_target or workflow_run workflow appears to check out contributor-controlled PR head code.",
      evidence: [
        workflow.triggers.includes("pull_request_target")
      ...
      recommendation:
        "Do not execute fork PR head code in privileged workflows. Check out the base ref for analysis, or split untrusted analysis and privileged writes into separate workflows.",
      references: [
