...
export const untrustedContextToAgentPromptRule: Rule = ({ workflow }) => {
  const findings = [];
    ...
    for (const [inputName, inputValue] of Object.entries(aiStep.step.with ?? {})) {
      ...
      findings.push(
        result({
          ...
          recommendation:
            "Do not pass raw issue, PR, comment, or commit content directly as agent instructions. Quote it as data, summarize in a read-only phase, or require maintainer approval before privileged steps.",
          references: [
