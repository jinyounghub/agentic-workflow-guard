...
export const missingExplicitPermissionsRule: Rule = ({ workflow }) => {
  const findings = [];
  ...
  for (const aiStep of workflow.aiSteps) {
    ...
    findings.push(
      result({
        ...
        recommendation:
          "Declare top-level permissions: contents: read, then grant job-specific write scopes only where necessary.",
        references: [
