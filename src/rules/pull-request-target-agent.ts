...
export const pullRequestTargetAgentRule: Rule = ({ workflow }) => {
  ...
  return workflow.aiSteps.map((aiStep) => {
    ...
    return result({
      ...
      message:
        "AI action executes in a pull_request_target workflow, where base repository token and secret boundaries are easy to misuse.",
      evidence: [
      ...
      recommendation:
        "Prefer pull_request for untrusted PR analysis. If pull_request_target is required, avoid checking out fork code and keep AI steps read-only with explicit maintainer approval before writes.",
      references: [
