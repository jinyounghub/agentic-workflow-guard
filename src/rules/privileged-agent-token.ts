...
export const privilegedAgentTokenRule: Rule = ({ workflow }) => {
  const findings = [];
  ...
  for (const aiStep of workflow.aiSteps) {
    ...
    findings.push(
      result({
        ...
        recommendation:
          "Start with top-level permissions: contents: read, then grant write scopes only to narrow non-AI jobs after explicit review gates.",
        references: [
